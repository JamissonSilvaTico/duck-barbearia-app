...
export const createAppointment = async (req: express.Request, res: express.Response) => {
  ...
  if (!customer || !customer.name || !customer.phone || !serviceId || !startTime) {
  ...
  try {
    ...
    if (existingAppointment) {
      await session.abortTransaction();
      session.endSession();
      return res.status(409).json({ msg: 'Este horário não está mais disponível.' });
    }
...
// @access  Público
export const getAvailability = async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        for (let t = openingTime; t <= closingTime - serviceDuration; t += interval) {
            ...
            if (!isOverlapping) {
                availableSlots.push(slotStart.toLocaleTimeString('pt-BR', { timeZone: 'America/Sao_Paulo', hour: '2-digit', minute: '2-digit' }));
            }
        }
...
// @access  Privado (Admin)
export const deleteAppointment = async (req: express.Request, res: express.Response) => {
  try {
