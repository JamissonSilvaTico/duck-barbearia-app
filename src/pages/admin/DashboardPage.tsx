...
const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      ...
      try {
        ...
        const appointmentsData = await api<PopulatedAppointment[]>(
  ...
  const totalRevenue = useMemo(() => {
    return appointments.reduce(
      (total, app) => total + (app.service?.price || 0),
      0
    );
  }, [appointments]);
  ...
  }> = ({ value, current, setter, children }) => (
    <button
      onClick={() => setter(value)}
      className={`px-4 py-2 text-sm font-medium rounded-md transition ${
        current === value
          ? "bg-brand-gold text-brand-dark"
          : "bg-brand-light-dark hover:bg-brand-gold/20"
      }`}
    >
      {children}
    </button>
    ...
    <div>
      ...
      <div className="bg-brand-dark p-6 rounded-lg mb-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            <label className="text-sm font-medium text-brand-gray mb-2 block">
              Filtrar por Serviço
            </label>
            <select
              onChange={(e) => setFilterService(e.target.value)}
              value={filterService}
              className="w-full bg-brand-light-dark p-2 rounded-md border border-brand-gold/30 focus:outline-none focus:ring-1 focus:ring-brand-gold"
            >
      ...
      <div className="bg-brand-dark p-6 rounded-lg mb-6">
        ...
        <p className="text-sm text-brand-gray">
          Total para o período e filtros selecionados.
        </p>
      </div>
            ...
            {loading ? (
              ...
              appointments.map((app) => {
                return (
                  ...
                  >
                    <td className="p-4">
                      {app.customer?.name || "Cliente removido"}
                    </td>
                    <td className="p-4">
                      {app.service?.name || "Serviço removido"}
                    </td>
                    <td className="p-4">
                      {new Date(app.startTime).toLocaleString("pt-BR")}
                    </td>
                    <td className="p-4 text-brand-gold">
                      R$ {app.service?.price.toFixed(2) || "N/A"}
                    </td>
                    <td className="p-4">
                      <button
